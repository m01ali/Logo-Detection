...
def review_known_logos_tab(db, AUGMENTATIONS=None): 
    ...
    # Brand filter + selection seeding
    ...
    if not filtered_df.empty:
        ...
        # === NEW: Bulk controls ABOVE the images ===
        ...
        # === NEW: Export Updated CSV controls (moved above the images) ===
        st.markdown("📤 **Export Updated CSV**")

        # Apply any current in-memory changes from known_brand_buffer before exporting
        export_df = df.copy()
        ...
        # Create or update correction column
        ...
        for _, row in filtered_df.iterrows():
            ...
            if select_all:
                ...
                # if the user just unchecked "Select all", clear all visible selections
                st.session_state.bulk_selection[path_str] = False
        ...
        for i, (_, row) in enumerate(filtered_df.iterrows()):
            with cols[i % 4]:
                ...
                # per-item quick actions (unchanged)
                ...
                with btn_col2:
                    # FALSE_LOGO instead of deletion
